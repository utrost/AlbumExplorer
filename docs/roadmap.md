...
Status: first executable slice shipped in `src/data/derived-relationships.js` and the album detail panel. Current browser output derives strong related-album suggestions from shared labels and full-scope Discogs credit candidates: producers, engineers, studios, songwriters, and musicians/performers. Broad genres/tags, Rolling Stone list editions, and adjacent release periods remain supported by the deterministic model, but the live startup UI does not materialize those broad edge classes yet because they create a sluggish 189k-edge hairball without density controls. Relationship records keep typed explanation entries so filtered views can foreground the reason that matched the active relationship type. Credit/studio-derived explanations now also keep Discogs provenance objects so the UI can show compact source badges and link back to release/master evidence when available. The Discogs credit import now covers all 760 comparison albums with 428 candidate albums and a generated 332-item review queue for remaining overrides/search aliases.
